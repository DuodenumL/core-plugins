...
func cmdUpdateNodeResourceCapacity(c *cli.Context) error {
	// volume update-capacity --node xxx-node --resource-opts {"cpu": 1} --incr
	...
	if err = volume.UpdateNodeResourceCapacity(c.Context, node, resourceOpts, incr); err != nil {
...
func cmdUpdateNodeResourceUsage(c *cli.Context) error {
	// volume update-usage --node xxx-node --resource-args {"cpu": 1} --incr
	volume, err := newVolume(c)
	if err != nil {
		return err
	}
	...
	if err = volume.UpdateNodeResourceUsage(c.Context, node, resourceArgsList, incr); err != nil {
