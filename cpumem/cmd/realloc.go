...
func cmdRealloc(c *cli.Context) error {
	// cpumem realloc --node xxx-node --old {"cpu": 1} --resource-opts {"cpu": 2}
	...
	engineArgs, deltaWorkloadResourceArgs, finalWorkloadResourceArgs, err := cpumem.Realloc(c.Context, node, oldResourceArgs, newResourceOpts)
	if err != nil {
