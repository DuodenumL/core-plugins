...
func cmdSetNode(c *cli.Context) error {
	// cpumem set-node --node xxx-node --capacity {"cpu": 100} --usage {"cpu": 20}
	...
	err = cpumem.SetNodeResourceInfo(c.Context, node, resourceCapacity, resourceUsage)
	if err != nil {
...
func cmdGetNode(c *cli.Context) error {
	// cpumem get-node --node xxx-node --workload-map {"workload-1": {"cpu": 100}} --fix
	cpumem, err := newCPUMem(c)
	if err != nil {
		return err
	}
	...
	nodeResourceInfo, diffs, err := cpumem.GetNodeResourceInfo(c.Context, node, workloadMap, fix)
	if err != nil {
