...
func TestAlloc(t *testing.T) {
	...
	// invalid opts
	_, _, err := cpuMem.Alloc(ctx, node, 1, &types.WorkloadResourceOpts{
		CPUBind:    true,
		CPURequest: -1,
	})
	...
	// non-existent node
	_, _, err = cpuMem.Alloc(ctx, "xxx", 1, &types.WorkloadResourceOpts{
		CPUBind:    true,
		CPURequest: 1,
	})
	...
	_, _, err = cpuMem.Alloc(ctx, node, 3, &types.WorkloadResourceOpts{
		CPUBind:    true,
		CPURequest: 1,
	})
	assert.True(t, errors.Is(err, types.ErrInsufficientResource))
	...
	_, resourceArgs, err := cpuMem.Alloc(ctx, "numa-node", 2, &types.WorkloadResourceOpts{
		CPUBind: true,
